...
// Versión completa con validación de dorsales únicos y todos los campos visibles
...
// ✅ Función para extraer dorsales y detectar duplicados
function extractDorsals($player_ids, $custom_numbers, $players_by_team, $team_id) {
    $dorsales = [];
    $count = max(count($player_ids), count($custom_numbers));
    for ($i = 0; $i < $count; $i++) {
        $pid = $player_ids[$i] ?? null;
        $cnum = $custom_numbers[$i] ?? '';
        $dorsal = '';
        if ($pid && isset($players_by_team[(int)$team_id])) {
            foreach ($players_by_team[(int)$team_id] as $p) {
                if ((int)$p['id'] === (int)$pid) {
                    $dorsal = $p['number'] ?? '';
                    break;
                }
...
function hasDuplicateDorsals($dorsales) {
    return count($dorsales) !== count(array_unique($dorsales));
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrf_check($_POST['csrf_token'] ?? '')) {
        ...
        if (!$home || !$away || !$date) {
            ...
            // Cargar jugadores por equipo para validación
            $teams = [$home, $away];
            $teamIds = array_filter($teams);
            $players_by_team = [];
            if (!empty($teamIds)) {
                $placeholders = implode(',', array_fill(0, count($teamIds), '?'));
                $stmt = $pdo->prepare("SELECT id, team_id, number, first_name, last_name FROM players WHERE team_id IN ($placeholders) ORDER BY (number+0) ASC, number ASC, last_name ASC");
            ...
            $home_dorsales = extractDorsals($home_player_ids, $home_custom_numbers, $players_by_team, $home);
            $away_dorsales = extractDorsals($away_player_ids, $away_custom_numbers, $players_by_team, $away);

            if (hasDuplicateDorsals($home_dorsales)) {
                $errors[] = 'El equipo local tiene dorsales duplicados.';
            }
            if (hasDuplicateDorsals($away_dorsales)) {
                $errors[] = 'El equipo visitante tiene dorsales duplicados.';
            }
...
// Load existing match_players for edit
$home_existing = [];
$away_existing = [];
if ($editId) {
...
<head>
  ...
  <style>
    ...
    .btn { padding: 12px 24px; font-size: 16px; border: none; border-radius: 8px; cursor: pointer; }
    .btn-primary { background: #1e3a8a; color: white; }
    .btn-secondary { background: #6b7280; color: white; }
    .alert { padding: 12px; margin-bottom: 16px; border-radius: 8px; }
    .alert-error { background: #fee2e2; color: #b91c1c; }
    .alert-success { background: #d1fae5; color: #065f46; }
    .slot-controls { display: flex; justify-content: space-between; margin-top: 10px; }
    ...
    .back-links a { display: inline-block; margin-right: 10px; }
  </style>
</head>
<body>
  <div class="back-links">
    <a href="<?=htmlspecialchars(url('dashboard.php'))?>" class="btn btn-secondary">&larr; Dashboard</a>
    <a href="<?=htmlspecialchars(url('matches.php'))?>" class="btn btn-secondary">&larr; Partidos</a>
  </div>
  ...
  <form method="post" enctype="multipart/form-data">
    ...
    <!-- JUGADORES - LOCAL -->
    <div style="background:white; padding:20px; border-radius:12px; margin:24px 0;">
      <div class="team-header">
        <h3 style="margin:0;"><?= htmlspecialchars($homeTeamName ?: 'Jugadores - Local') ?></h3>
        <button type="button" id="add-home-player" class="btn btn-secondary" style="padding:6px 12px;">➕ Añadir</button>
      </div>
      <div id="home-players-list">
        ...
        ?>
        <div class="slot" style="background:#f9fafb; padding:16px; margin:12px 0; border-radius:10px;">
          <select name="home_player_id[]" style="margin-bottom:10px;">
          ...
          <input type="text" name="home_player_custom_number[]" placeholder="Nº (opcional)" value="<?=htmlspecialchars($preset_num)?>" style="margin-bottom:10px;">
          <input type="text" name="home_player_custom_name[]" placeholder="Nombre (opcional)" value="<?=htmlspecialchars(trim(($preset_first . ' ' . $preset_last) ?: ''))?>" style="margin-bottom:10px;">
          <textarea name="player_notes_home[]" rows="2" placeholder="Notas jugador..."><?=htmlspecialchars($preset_notes)?></textarea>
          <div class="slot-controls">
            <label style="display:flex;align-items:center;">
              <input type="checkbox" name="home_is_starter[<?= $i ?>]" value="1" <?= $preset_is_starter ? 'checked' : '' ?>> Titular
            </label>
            <button type="button" class="btn btn-secondary remove-player" style="padding:6px 12px;">🗑️ Eliminar</button>
          </div>
        </div>
        <?php endfor; ?>
      </div>
    ...
    <div style="background:white; padding:20px; border-radius:12px; margin:24px 0;">
      <div class="team-header">
        <h3 style="margin:0;"><?= htmlspecialchars($awayTeamName ?: 'Jugadores - Visitante') ?></h3>
        <button type="button" id="add-away-player" class="btn btn-secondary" style="padding:6px 12px;">➕ Añadir</button>
      </div>
      <div id="away-players-list">
        ...
        ?>
        <div class="slot" style="background:#f9fafb; padding:16px; margin:12px 0; border-radius:10px;">
          <select name="away_player_id[]" style="margin-bottom:10px;">
            <option value="">-- seleccionar jugador --</option>
            <?php
          ...
          <input type="text" name="away_player_custom_number[]" placeholder="Nº (opcional)" value="<?=htmlspecialchars($preset_num)?>" style="margin-bottom:10px;">
          <input type="text" name="away_player_custom_name[]" placeholder="Nombre (opcional)" value="<?=htmlspecialchars(trim(($preset_first . ' ' . $preset_last) ?: ''))?>" style="margin-bottom:10px;">
          <textarea name="player_notes_away[]" rows="2" placeholder="Notas jugador..."><?=htmlspecialchars($preset_notes)?></textarea>
          <div class="slot-controls">
            <label style="display:flex;align-items:center;">
              <input type="checkbox" name="away_is_starter[<?= $i ?>]" value="1" <?= $preset_is_starter ? 'checked' : '' ?>> Titular
            </label>
            <button type="button" class="btn btn-secondary remove-player" style="padding:6px 12px;">🗑️ Eliminar</button>
          </div>
        </div>
    ...
    <!-- Cuartos -->
    <div style="background:white; padding:20px; border-radius:12px; margin:24px 0;">
      <h3 style="margin-bottom:16px;">Puntuación por cuartos</h3>
      <div style="display:flex; flex-wrap:wrap; gap:12px;">
        ...
        ?>
          <div>
            <div style="font-weight:bold; text-align:center;">Q<?= $i+1 ?></div>
            <input type="number" min="0" name="home_q[]" placeholder="Local" value="<?= htmlspecialchars($h) ?>" style="width:100px; margin:4px;">
            <input type="number" min="0" name="away_q[]" placeholder="Visitante" value="<?= htmlspecialchars($a) ?>" style="width:100px; margin:4px;">
          </div>
        <?php endfor; ?>
      </div>
      <div style="margin-top:16px;">
        <label>Puntuación total (si difiere)</label>
    ...
    <div class="form-group">
      ...
      <div class="form-text" style="font-size:14px; color:#6b7280; margin-top:6px;">
  ...
  <script>
    ...
    document.getElementById('add-home-player')?.addEventListener('click', () => {
      const list = document.getElementById('home-players-list');
      const count = list.querySelectorAll('.slot').length;
      const div = document.createElement('div');
      div.className = 'slot';
      div.style = 'background:#f9fafb; padding:16px; margin:12px 0; border-radius:10px;';
      div.innerHTML = `
        <select name="home_player_id[]" style="margin-bottom:10px;">
          <option value="">-- seleccionar jugador --</option>
        </select>
        <input type="text" name="home_player_custom_number[]" placeholder="Nº (opcional)" style="margin-bottom:10px;">
        <input type="text" name="home_player_custom_name[]" placeholder="Nombre (opcional)" style="margin-bottom:10px;">
        <textarea name="player_notes_home[]" rows="2" placeholder="Notas jugador..."></textarea>
        <div class="slot-controls">
          <label style="display:flex;align-items:center;">
            <input type="checkbox" name="home_is_starter[${count}]" value="1"> Titular
          </label>
          <button type="button" class="btn btn-secondary remove-player" style="padding:6px 12px;">🗑️ Eliminar</button>
        </div>`;
      list.appendChild(div);
      div.querySelector('.remove-player').addEventListener('click', removeSlotHandler);
    });

    document.getElementById('add-away-player')?.addEventListener('click', () => {
      const list = document.getElementById('away-players-list');
      const count = list.querySelectorAll('.slot').length;
      const div = document.createElement('div');
      div.className = 'slot';
      div.style = 'background:#f9fafb; padding:16px; margin:12px 0; border-radius:10px;';
      div.innerHTML = `
        ...
        <input type="text" name="away_player_custom_number[]" placeholder="Nº (opcional)" style="margin-bottom:10px;">
        <input type="text" name="away_player_custom_name[]" placeholder="Nombre (opcional)" style="margin-bottom:10px;">
        <textarea name="player_notes_away[]" rows="2" placeholder="Notas jugador..."></textarea>
        <div class="slot-controls">
          <label style="display:flex;align-items:center;">
            <input type="checkbox" name="away_is_starter[${count}]" value="1"> Titular
          </label>
          <button type="button" class="btn btn-secondary remove-player" style="padding:6px 12px;">🗑️ Eliminar</button>
        </div>`;
      list.appendChild(div);
