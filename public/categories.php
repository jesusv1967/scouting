      ...
      <?php if ($categoryToEdit): ?>
        ...
        <form method="post">
          ...
          <div class="mb-3">
            ...
            <input name="name" class="form-control" required value="<?=htmlspecialchars($categoryToEdit['name'])?>">
          </div>
          <div class="mb-3">
            <label class="form-label">Descripción</label>
            <textarea name="description" class="form-control"><?=htmlspecialchars($categoryToEdit['description'])?></textarea>
          </div>
          <div class="d-flex gap-2">
            <button class="btn btn-primary">Actualizar</button>
            <a href="<?=htmlspecialchars(url('categories.php'))?>" class="btn btn-outline-secondary">Cancelar</a>
          </div>
        </form>

        <form method="post" class="mt-3" onsubmit="return confirm('¿Eliminar esta categoría?');">
          <input type="hidden" name="csrf_token" value="<?=htmlspecialchars($csrf)?>">
          <input type="hidden" name="action" value="delete">
          <input type="hidden" name="id" value="<?=htmlspecialchars($categoryToEdit['id'])?>">
          <button class="btn btn-outline-danger">Eliminar categoría</button>
        </form>
      <?php else: ?>
      ...
      <table class="table table-sm">
        ...
        <?php foreach($categories as $c): ?>
          <tr>
            ...
            <td class="text-end">
              <a href="<?=htmlspecialchars(url('categories.php') . '?id=' . $c['id'])?>" class="btn btn-sm btn-outline-primary" title="Editar"><i class="bi bi-pencil"></i></a>
            </td>
          </tr>
        <?php endforeach; ?>
        </tbody>
